...
int compute_distance(pii x, pii y){
    return ((abs(x.ff - y.ff) * abs(x.ff - y.ff)) + (abs(x.ss - y.ss) * abs(x.ss - y.ss)));
}
void solution()
{
    int n;
    ...
    int distance = compute_distance(start, end);
    
    for(auto x : arr){
        int cur_dis = compute_distance(x, end);
        if(distance>=cur_dis){
            no;
            return;
        }
