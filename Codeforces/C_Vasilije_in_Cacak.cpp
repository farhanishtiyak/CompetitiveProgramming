...
void solution()
{
    ll n,k,x;
    cin>>n>>k>>x;
    ll forwardSum=0, backwardSum = 0;
    
    // for(ll i=1; i<=k; i++) forwardSum+=i;
    // ll cnt = k;
    // ll val = n;
    // while(cnt){
    //     backwardSum+=val;
    ...
    ll first = ((n-k)*(n-k+1))/2;
    backwardSum = totalSum- first;

    // cout<<"Forward Sum = "<<forwardSum<<endl;
    // cout<<"Backward Sum = "<<backwardSum<<endl;

    if(x>=forwardSum and x<=backwardSum) cout<<"YES"<<endl;
    else cout<<"NO"<<endl;
