...
// int run(int cur, int par, int last){
//     if(gr[cur].size()==1){
//         int rating = arr[cur];
//         return rating > last;
//     }
...
//         if(x!=par){
//             //not take
//             int not_take = run(x, cur, last);
//             // take
//             int take = 0;

//             if(arr[x]>last){
//                 take = 1 + run(x, cur, arr[x]);
//             }
...
class Graph {
public:
    Graph(int n, const vector<int>& stars, const vector<pair<int, int>>& roads)
        : n(n), stars(stars), graph(n) {
        for (const auto& road : roads) {
            graph[road.first - 1].push_back(road.second - 1);
            graph[road.second - 1].push_back(road.first - 1);
        }
    }
    ...
    int maximizeDine() {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            ans = max(ans, dfs(i, -1, visited, stars[i], 1));
        }
        return ans;
    }
...
private:
    ...
    int dfs(int current, int lastStar, vector<bool>& visited, int currentStar, int count) {
        ...
        for (int neighbor : graph[current]) {
            if (!visited[neighbor]) {
                if (stars[neighbor] > currentStar) {
                    maxCount = max(maxCount, dfs(neighbor, currentStar, visited, stars[neighbor], count + 1));
                } else {
                    maxCount = max(maxCount, dfs(neighbor, lastStar, visited, currentStar, count));
                }
            }
        }
...
signed main() {
    ...
    Graph obj(N, stars, roads);
    cout << obj.maximizeDine() << endl;
